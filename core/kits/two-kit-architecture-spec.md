...
│       ├── archived/
│       └── state.json
├── .core-sync.json           # 仅 Hub 管理项目需要；独立项目可不存在或为空说明
├── .internal/
├── .obsidian/
├── .agents/skills/
├── .claude/skills/
...
|---|---|---|---|
| 独立 Project | 没有 `hub` 字段 | 可不存在 | 本地维护 |
| Satellite Project | 有 `hub.path` 和 `hub.project_id` | 必须存在 | 来自 Hub 的快照或链接 |

也就是说：

```text
Project Kit + hub binding = Satellite
...
- `project` Kit 同时能服务独立项目和 Satellite。
- Satellite 相关内容通过 `.core-sync.json` 和 `hub` state 表达。
