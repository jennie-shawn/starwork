...
Matter Mode 解决的是“一个事项如何长期推进、记录和归档”。
...
如果把每个 Agent 都建成一个 matter，会把机制做重，也会混淆“事情”和“职责”。因此 Agent Lanes 独立于 Matter Mode：它是协作层，不是事项层。
...
| `purpose` | 该 lane 的职责。 |
| `current_session` | 当前绑定的会话；没有绑定时写 `unbound`。 |
...
|---|---|---|---|---|
| research | 资料收集和事实核查 | codex:019e... | knowledge/**, notes/** | lanes/research/worklog.md |
| writing | 草稿撰写和结构整理 | unbound | drafts/**, product/docs/** | lanes/writing/worklog.md |
| review | 审校、验证和风险检查 | unbound | reviews/**, product/docs/** | lanes/review/worklog.md |
```
...
v0.1 不默认创建 lane 专属 `outputs/`、`drafts/`、`notes/` 目录。

如果某个 lane 确实需要独立产物目录，可以按需新增：

```text
_系统/协作/lanes/<lane-id>/artifacts/
```
...
| 临时但独立的协作产物 | 按需放入对应 lane 的 `artifacts/` |
...
- 更新 `agent-lanes.md` 中对应 lane 的 `current_session`。
...
Agent Lanes 不属于 Matter Mode。

- Matter 追踪事情。
- Lane 追踪职责。

二者可以同时存在，但互不替代。一个 lane 可以参与多个 matter，一个 matter 也可以需要多个 lane 协作。
...
- 不启用 Matter Mode 的单事务项目也能使用 Agent Lanes。

## 后续问题

这些问题暂不进入 v0.1：

- 是否需要机器可读 `.starwork/agent-lanes.json`。
- 是否需要 lane 级别的默认验证命令。
