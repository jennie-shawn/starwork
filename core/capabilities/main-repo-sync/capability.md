...
- `_系统/上下文/当前项目.md` 或 `_system/context/current-projects.md`，作为项目状态事实源
...
| 机制 | 用途 | 是否改变 Hub 正式内容 |
|---|---|---:|
| `.starwork/incoming/` | Satellite 向 Hub 提交候选身份、教训、报告或知识更新。 | 只有 Hub 审核后才会改变。 |
| `.starwork/handoff/` | 一个项目向另一个项目发送请求、同步、依赖或回复。 | 不会。 |

候选内容一旦被审核通过，应进入 Hub 共享资产目录，例如 `identity/`、`lessons/`、`knowledge/` 或 `skills/`。`.starwork/incoming/` 不应成为长期内容库。
