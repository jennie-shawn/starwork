...
- 命令：`starwork spawn`
- 实现状态：v0.1 第一版已实现
- 相关对象：`hub`、`satellite-starter`、`project`、`main-repo-sync`、`skill-mount`
...
- Hub 的 `projects/registry.json` 中新增该项目记录。
...
| `--mode <project>` | 卫星项目模式。默认 `project`。 |
| `--language <zh|en>` | 卫星项目语言。默认继承 Hub；也可由 blueprint 指定。 |
...
| `project` | `project` | 接入主库的项目卫星项目，使用 Project 标准结构。 |
...
- Hub 中必须存在项目注册表：`projects/registry.json`。
- Hub 中必须存在共享资源入口：`identity/`、`lessons/`、`skills/`、`.incoming/`、`knowledge/`。
...
这两类是独立工作区类型，不复用 `single-light` / `project`。
...
不能把 Hub 的 `projects/registry.json` 当成项目进度正文。
...
- Hub `projects/registry.json` 出现项目记录。
...
4. 是否需要在 Hub `projects/coordination/` 自动生成项目创建 handoff。
5. 软链接失败时是否提供复制降级策略。
