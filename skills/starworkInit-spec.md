...
```text
建议使用：single-light
基础 Kit：local-starter
语言：zh
Pack：general
正式成果：输出/确认成果/
当前工作：参考资料/
额外目录：按用户需求另行定制
原因：用户当前只描述了一个明确项目，先用单事务项目降低复杂度。
```
...
仅当 Step 1 判断为 `single-light` 或 `project` 候选时，才进入这一步。
...
- 明确有多个事项、多个阶段、多个交付线需要分别推进：`project`
...
| `workspace_type` | `single-light`、`project` 或 `hub`。 |
| `kit` | `local-starter`、`project` 或 `hub`。 |
