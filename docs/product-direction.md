...
截至 2026-05-18，Core 被定义为开源 AI 工作区协议；Kit 是协议的参考落地结构；CLI 负责稳定生成、检查、适配和安装 Pack；Skill 负责帮助 Agent 生成配置、理解流程和调用工具；Pack 负责场景定制。
...
下一步重点不是继续扩张 Core，而是验证 CLI、Skill 和 Pack 能否在真实场景里稳定生成、检查、安装和解释这些结构。
